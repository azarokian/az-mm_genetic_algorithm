...
class GeneticAlgorithm:
    def __init__(self, iterations, population_size, cities, elites_num, mutation_rate,
                 greedy_seed=0, roulette_selection=True, plot_progress=True, store=None):
    ...
    def plot(self, label=None):
        ...
        x_store = self.store[0]
        y_store = self.store[1]

        x_list.append(self.best_chromosome()[0].x)
        y_list.append(self.best_chromosome()[0].y)
        fig = plt.figure(1)
        fig.clear()
        fig.suptitle(f'Route for {label}')

        plt.xlim(0, 100)  # setting x-axis limit from 0 to 6
        plt.ylim(0, 100)  # setting y-axis limit from 0 to 12

        plt.plot(x_list, y_list, 'ro')
        plt.plot(x_list, y_list, 'g')
        plt.plot(x_store, y_store, 'bs')
