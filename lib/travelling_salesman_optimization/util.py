...
def read_cities(size):
    cities = []
    with open(f'/home/az/order_sequencing_ga/lib/Travelling_Salesman_Optimization/test_data/cities_{size}.data', 'r') as handle:
...
def read_orders(order_list: list):
    orders = []
    for order in order_list:
        x, y = float(order[0]), float(order[1])
        orders.append(City(x, y))
    return orders


def write_cities_and_return_them(size):
    cities = generate_cities(size)
    with open(f'/home/az/order_sequencing_ga/lib/Travelling_Salesman_Optimization/test_data/cities_{size}.data', 'w+') as handle:
        for city in cities:
